...
H_values = np.linspace(1, 2, values)
...
for s_i in range(int(size/steps)):
    ...
    for variable in range(vars):#,9,0,1,7]
        ...
        for t in np.arange(len(all_structures[variable])):
            for i in np.arange(len(all_structures[variable][t])):
                ...
                for j in np.arange(len(all_structures[variable][t][i][0,:])):
                    ...
                    x_coord = structure_x[j]
                    matrix_t[t, x_coord,y_coord] = i + 1
                    frequency_matrix[x_coord,y_coord] =  frequency_matrix[x_coord,y_coord] + 1
        ...
        plt.savefig(f'testing/{shap_name}/structures_plot_with_object_{variable}_perc_{ H_values[variable]}.png', bbox_inches='tight')
        plt.close()
        ...
        # Save the plot
        plt.savefig(f'testing/{shap_name}/frequency_matrix_{variable}.png', bbox_inches='tight')
        plt.close()
