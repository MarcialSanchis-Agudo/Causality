...
if trainer.is_global_zero:
    ...
    dataset.add_field('u_x', 1, u_x.numpy().reshape((len(time), nx * ny * nz)).T)
    dataset.add_field('u_y', 1, u_y.numpy().reshape((len(time), nx * ny * nz)).T)
    dataset.add_field('u_z', 1, u_z.numpy().reshape((len(time), nx * ny * nz)).T)
